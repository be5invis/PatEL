    ...
    if (ex.begins >= 0 && ex.ends >= 0) {
        ...
        if (ex.within && ex.within.input) {
            ...
            if (ljEnds - ljBegins < 3) {
                printSourceLines(
                    stderr,
                    lines.slice(ljBegins, ljEnds + 1),
                    ljBegins + 1
                );
            } else {
                printSourceLines(
                    stderr,
                    lines.slice(ljBegins, ljBegins + 2),
                    ljBegins + 1
                );
                stderr.write("......\n");
                printSourceLines(
                    stderr,
                    lines.slice(ljEnds - 1, ljEnds + 1),
                    ljEnds
                );
            }
        }
...
function compile(_input, _options, callback) {
    ...
    try {
        ...
        if (options["dump-expanded"]) {
            return callback(null, null, xast);
        }

        options.strict = options.strict || isStrict;

        var rast = patel.regularize(xast, gs);
        if (options["dump-regularized"]) {
